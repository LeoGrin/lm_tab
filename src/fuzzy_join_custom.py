...
def fuzzy_join(
    ...
    main_str_enc_arg=None,
    aux_str_enc_arg=None,
) -> pd.DataFrame:
    ...
    main_enc, aux_enc = [], []
    if any_numeric:
        assert main_str_enc_arg is None and aux_str_enc_arg is None
    ...
    if any_str:
        if main_str_enc_arg is None:
            assert aux_str_enc_arg is None
            ...
            main_str_enc = main_str_enc_arg
            aux_str_enc = aux_str_enc_arg
        main_enc.append(main_str_enc)
        aux_enc.append(aux_str_enc)
    if main_str_enc_arg is None:
        main_enc = hstack(main_enc, format="csr") #TODO: not always csr
        aux_enc = hstack(aux_enc, format="csr")
    else:
        main_enc = np.hstack(main_enc)
        aux_enc = np.hstack(aux_enc)
