...
def run_with_hv(dataset, analyzer, ngram_range, dim_reduction_name, dim_reduction, cv, model_name, model, features, cardinality_threshold=30):
...
def pipeline(config):
    ...
    dataset, analyzer, ngram_range, dim_reduction_name, model_name, features, n_train = config
    n_test = 500
    ...
    return run_with_hv(dataset, analyzer, ngram_range, dim_reduction_name, dim_reduction, cv, model_name, model, features)
...
param_combinations = list(product(datasets, analyzers, ngram_ranges, dim_reduction_names, model_names, features_list, n_trains))
...
name = "results_hashing_vectorizer_22_11"
for job in jobs:
    try:
        result = job.result()
